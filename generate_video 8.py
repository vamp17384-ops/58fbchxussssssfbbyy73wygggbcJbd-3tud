...
async def _run_tts(text: str, mp3_path: str, vtt_path: str):
    ...
    # edge-tts 7.x: SubMaker.generate_subs() returns SRT string
    # Try multiple API names for compatibility
    srt_content = None
    for method in ["get_srt", "generate_subs", "srt"]:
        if hasattr(submaker, method):
            result = getattr(submaker, method)()
            if result and isinstance(result, str):
                srt_content = result
                print(f"  Used submaker.{method}()")
                break
    if srt_content is None:
        # Fallback: build SRT from word boundaries manually
        print("  WARNING: no submaker method found, building SRT from scratch")
        srt_content = ""
...
def parse_srt(srt_path: str):
    ...
    # Normalise line endings and print first bit for debugging
    text = raw.replace("\r\n", "\n").replace("\r", "\n")
    ...
    for block in blocks:
        ...
        if len(lines) < 2:
            continue
        # Find the timecode line (may be line 0 or 1)
...
def compose_video(bg: str, audio: str, srt: str, duration: float, out: str):
    """Compose final video. Uses drawtext per word — no subtitles filter path issues."""
    ...
    base = (
        ...
        f"eq=brightness=-0.05:saturation=1.1[base]"
    )

    # Use a Python script passed via -vf to write a filtergraph file
    # Actually: write the entire complex filtergraph to a temp file and pass with -filter_complex_script
    # This avoids ALL shell escaping issues completely
    import tempfile, os

    # Build filtergraph — drawtext reads text from a textfile= to avoid escaping
    # For each word, write its text to a temp file and reference that
    word_files = []
    dt_parts   = []

    for i, (start, end, word) in enumerate(entries):
        # Write word to a temp text file — bypasses all escaping
        wf = f"/tmp/word_{i:04d}.txt"
        open(wf, "w").write(word)
    ...
    # Write filtergraph script to file
    fg_lines = [base]
    # Chain drawtext filters on [base]
    if dt_parts:
        prev = "[base]"
        for i, dt in enumerate(dt_parts):
            next_label = f"[dt{i}]" if i < len(dt_parts) - 1 else "[out]"
            fg_lines.append(f"{prev}{dt}{next_label}")
            prev = next_label
    else:
        # No subtitle entries — just pass through
        fg_lines.append("[base]null[out]")

    fg_script = "\n".join(fg_lines)
    fg_file = "/tmp/filtergraph.txt"
    open(fg_file, "w").write(fg_script)
    print(f"  Filter script: {len(fg_lines)} lines")
