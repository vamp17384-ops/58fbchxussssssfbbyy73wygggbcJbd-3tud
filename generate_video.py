...
def compose_video(bg: str, audio: str, srt: str, duration: float, out: str):
    import shutil
    # FFmpeg subtitles filter cannot read from /tmp — copy SRT to cwd
    srt_local = "subs_render.srt"
    shutil.copy(srt, srt_local)
    srt_esc = srt_local.replace("\\", "/").replace(":", "\\:")

    fg = (
        # Scale/crop background to 9:16
        f"[0:v]"
        ...
        # Word-synced captions — dead center (Alignment=5), one word at a time
        f"[bg]subtitles='{srt_esc}':"
        f"force_style='"
        f"FontName=DejaVu Sans Bold,"
        f"FontSize=90,"           # big single word
        f"PrimaryColour=&H00FFFFFF,"
        f"OutlineColour=&H00000000,"
        f"BackColour=&HAA000000,"
        f"BorderStyle=4,"         # box behind word
        f"Outline=0,Shadow=0,"
        f"Alignment=5,"           # 5 = dead center of screen
        f"MarginV=0,MarginL=60,MarginR=60"
        f"'"
        f"[out]"
    )
    ...
    cmd = [
        "ffmpeg", "-y",
        "-stream_loop", "-1", "-i", bg,   # loop bg as long as needed
        "-i", audio,
        "-filter_complex", fg,
        "-map", "[out]", "-map", "1:a",
        "-t", str(duration + 0.3),         # exact audio length, no cap
